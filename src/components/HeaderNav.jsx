...
export default function HeaderNav() {
  ...
  const handleLogout = async () => {
    ...
    // ⬇ FIX PENTING AGAR SETELAH LOGOUT TIDAK MASUK KE DASHBOARD LAGI
    sessionStorage.removeItem("lastPageBeforeRefresh")
    ...
    <header className="bg-dark-red text-cream shadow-2xl fixed top-0 w-full z-50">
      <div className="flex items-center justify-between px-6 py-4 min-h-[80px]">
        ...
        <div className="flex items-center gap-3">
          ...
          {isDashboard && !isAdmin && (
            <button
              onClick={() => navigate('/animals')}
              className="bg-gradient-to-r from-orange-600 to-red-700 hover:from-orange-500 
                         hover:to-red-600 text-cream font-black text-xl px-8 py-4 rounded-full 
                         shadow-2xl flex items-center gap-3 hover:scale-110 transition-all duration-300"
            >
          ...
          {isAdmin && !isUploadPage && (
            <button
              ...
              className="bg-gradient-to-r from-green-600 to-emerald-700 hover:from-green-500 
                         hover:to-emerald-600 text-white font-black px-6 py-4 rounded-full 
                         shadow-2xl flex items-center gap-3 hover:scale-110 transition-all"
            >
              <Upload size={28} />
          ...
          {!isDashboard && !isUploadPage && (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 rounded-xl font-bold 
                          transition-all flex-row-reverse ${
                nextPage
                  ? 'bg-mid-red hover:bg-red-800 shadow-lg hover:scale-105'
                  : 'bg-dark-red/40 cursor-not-allowed opacity-50'
              }`}
      ...
      {!isDashboard && !isUploadPage && (
        <div className="h-2 bg-dark-red/40 overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-orange-500 via-red-600 to-beige 
